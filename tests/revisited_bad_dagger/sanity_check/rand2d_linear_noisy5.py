...
from policy import NoisyPolicy5 as EpsPolicy         # type of policy (plain, noisy, etc.)
import numpy as np
...
if __name__ == '__main__':
    ...
    for i, filename in enumerate(sorted(os.listdir('scenarios_sparse2d/'))):                          # path to pickled scenarios to test on
        if i >= 10:                                                                                    # in case you want to only test on subset of scenarios (recommended because faster)
            break
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/rand2d_linear_noisy5_' + str(filename)                            # path to optimal policy: convention for policy paths is 'policies/[description] + [scenario name]'
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('revisited/sanity_check/5/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
