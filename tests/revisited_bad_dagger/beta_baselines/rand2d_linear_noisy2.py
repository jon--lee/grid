...
from policy import NoisyPolicy2 as EpsPolicy         # type of policy (plain, noisy, etc.)
import numpy as np
    ...
    for i, filename in enumerate(sorted(os.listdir('scenarios_sparse2d/'))):                          # path to pickled scenarios to test on
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/rand2d_linear_noisy2_' + str(filename)                            # path to optimal policy: convention for policy paths is 'policies/[description] + [scenario name]'
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('revisited/beta_baseline7/2/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
