...
from policy import NoisyPolicy3 as EpsPolicy         # type of policy (plain, noisy, etc.)
import numpy as np
    ...
    for i, filename in enumerate(sorted(os.listdir('scenarios_sparse2d/'))):                          # path to pickled scenarios to test on
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                ...
                test = RandomTest('revisited/beta_baseline7/3/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
