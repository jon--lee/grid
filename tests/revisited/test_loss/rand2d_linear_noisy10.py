...
from policy import NoisyPolicy10         # type of policy (plain, noisy, etc.)
import numpy as np
    ...
    for filename in sorted(os.listdir('scenarios_sparse2d/')):                                      # path to pickled scenarios to test on
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                ...
                test = RandomTest('revisited/test_loss/10/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
