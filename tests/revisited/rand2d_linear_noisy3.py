...
from policy import NoisyPolicy3         # type of policy (plain, noisy, etc.)
import numpy as np
...
class RandomTest(BaseTest):
    ...
    def vanilla_supervise(self):
        mdp = ClassicMDP(NoisyPolicy3(self.grid), self.grid)
    ...
    def run(self, LIMIT_DATA, DEPTH, MOVES, scen, policy):
    ...
    for filename in sorted(os.listdir('scenarios_sparse2d/')):                                      # path to pickled scenarios to test on
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/rand2d_linear_noisy3_' + str(filename)                            # path to optimal policy: convention for policy paths is 'policies/[description] + [scenario name]'
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('revisited/test_loss/3/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
