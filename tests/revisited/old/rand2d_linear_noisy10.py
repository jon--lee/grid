...
from policy import NoisyPolicy10
import numpy as np
...
class RandomTest(BaseTest):
    ...
    def vanilla_supervise(self):
        mdp = ClassicMDP(NoisyPolicy10(self.grid), self.grid)
    ...
    def run(self, LIMIT_DATA, DEPTH, MOVES, scen, policy):
        ...
        #self.policy = 'policies/random2d_deter.p'
        self.policy = policy
...
if __name__ == '__main__':
    ...
    for filename in sorted(os.listdir('scenarios_sparse2d/')):
        if 'scen30' in filename:
            break
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/10random2d_deter_sparse_linear_noisy' + str(filename)
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('random2d_linear_noisy_revisited/10/' + filename, 40, ITER, TRIALS, SAMP)
