...
from policy import NoisyPolicy6
import numpy as np
...
class RandomTest(BaseTest):
    ...
    def vanilla_supervise(self):
        mdp = ClassicMDP(NoisyPolicy6(self.grid), self.grid)
    ...
    for filename in sorted(os.listdir('scenarios_sparse2d/')):
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/6random2d_deter_sparse_linear_noisy' + str(filename)
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('random2d_linear_noisy_revisited/6/' + filename, 40, ITER, TRIALS, SAMP)
