...
from policy import NoisyPolicy0
import numpy as np
    ...
    for filename in sorted(os.listdir('scenarios_sparse2d/')):
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/0random2d_deter_sparse_linear_noisy' + str(filename)
                scenario = random_scen.load('scenarios_sparse2d/' + filename)
                test = RandomTest('random2d_linear_noisy_revisited/0/' + filename, 40, ITER, TRIALS, SAMP)
