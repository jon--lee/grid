    ...
    for i, filename in enumerate(sorted(os.listdir('scenarios_sparse2d/'))):                          # path to pickled scenarios to test on
        ...
        for i in range(len(params)):
            if filename.endswith('.p'):
                ...
                test = RandomTest('revisited/eps_sweep_one_instance/0/' + filename, 40, ITER, TRIALS, SAMP)      # (ignore '40'), first parameter is where data/plots are saved within ./comparisons/
