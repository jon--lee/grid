...
class TestTest(BaseTest):
    ...
    def boosted_supervise(self):
        ...
        for t in range(self.TRIALS):
            ...
            mdp.load_policy(self.policy)
            dt = DecisionTreeClassifier(max_depth=self.DEPTH)
            boost = AdaBoostClassifier(dt, n_estimators=20)
    ...
    def run(self, LIMIT_DATA, DEPTH):
        ...
        self.policy = 'policies/4dim_policy6.p'
        ...
        analysis.plot(names = ['Value iteration', 'DT IL', 'DT DAgger', 'Adaboost IL', 'Adaboost DAgger'], filename=self.comparisons_directory + 'reward_comparison.eps',)
...
if __name__ == '__main__':
    ...
    test = TestTest('testing', 80, ITER, TRIALS, SAMP)
