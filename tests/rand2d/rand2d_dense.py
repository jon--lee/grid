...
if __name__ == '__main__':
    ...
    for filename in sorted(os.listdir('scenarios_dense2d/')):
        for i in range(len(params)):
            if filename.endswith('.p'):
                policy = 'policies/random2d_deter_dense_' + str(filename)
                scenario = random_scen.load('scenarios_dense2d/' + filename)
                test = RandomTest('random2d_deter_dense/' + filename, 40, ITER, TRIALS, SAMP)
