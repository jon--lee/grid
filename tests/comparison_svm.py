...
comparisons_directory = 'comparisons/svm_comparison/'
data_directory = 'comparisons/svm_data/'
...
for t in range(TRIALS):
    ...
    sup = ScikitSupervise(grid, mdp, Classifier=svm, moves=80)
...
analysis.plot(names=['Value Iteration', 'AdaBoost IL', 'RBF SVM DAgger'], 
        filename=comparisons_directory+'svm_reward_comparison.png', ylims=[-60, 100])
...
acc_analysis.plot(names = ['AdaBoost IL Acc.', 'RBF SVM DAgger'], label='Accuracy',
        filename=comparisons_directory+'svm_acc_comparison.png', ylims=[0,1])
